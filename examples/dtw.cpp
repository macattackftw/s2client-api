...
int main(int argc, char** argv)
{
    ...
    vector<vector<vector <float> > > strats;
        ...
        vector<string> lineb = GetLines(folder_name + csv_files[1]);

        vector<vector<float> > cheese(cols, vector<float>(linez.size(), 0.0f));
        vector<vector<float> > baseline(cols, vector<float>(linez.size(), 0.0f));
        ...
        for (unsigned int i = 0; i < linez.size(); ++i)
        {
            ...
            {
                pos = linez[i].rfind(",");
                cheese[j][i] = stof(linez[i].substr(pos + 1));
                linez[i].resize(pos);
            }
            // Deal with the first column
            cheese[0][i] = stof(linez[i]);
        }
        ...
        MultiDimensionalTimeWarping md_dtw(cheese, baseline);
